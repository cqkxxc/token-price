...
export interface Manifest { vendors?: { vendorId: string; file?: string; files?: string[] }[]; }

const ICON_ALIASES: Record<string, string> = {
  bytedance: 'doubao', volcengine: 'doubao', alibaba: 'qwen', 'alibaba-cloud': 'qwen',
  'zhipu-ai': 'zhipu', 'google-gemini': 'google', tencent: 'hunyuan',
};
...
export function iconFor(company: Company, manifest: Manifest): string {
...
export function modelLogo(m: Model, manifest: Manifest, size = ''): string {
  const icon = iconFor(m.company, manifest);
  const letter = esc((m.company.name || '?')[0].toUpperCase());
  const img = icon
    ? `<img src="${esc(icon)}" alt="${esc(m.company.name)} Logo" onerror="this.parentElement.classList.add('logo-fallback');this.remove()">`
    : '';
  return `<div class="model-avatar ${size}">${img}<span>${letter}</span></div>`;
}
